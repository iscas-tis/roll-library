...
public class TrimParserHOA implements Parser, HOAConsumer {

	// this class only allow the characters
	protected Automaton automaton;

	// ignored propositions
	public final static String TAIL = "_TAIL";
	// left labels
	...
	// if there are more than VAR_NUM_BOUND_TO_USE_BDD atomic propositions, then use
	// bdd
	protected final int VAR_NUM_BOUND_TO_USE_BDD = 7;
	protected NBA nba;
	protected final Options options;
	protected final Alphabet alphabet;
	// extra tail var should be ignored
	protected BDD tailVar = null;

	protected boolean initialAdded = false;

	public TrimParserHOA(Options options, String file) {
	        ...
	        try {
	            ...
	            BDD leftLabels = atomRemaining.not().andWith(tailVar.id());
	...
	protected TrimParserHOA(Options options) {
	        this.options = options;
	        this.alphabet = new Alphabet();
	}
	...
	public void addStartStates(List<Integer> stateConjunction) throws HOAConsumerException {
		if (initialAdded) {
			throw new UnsupportedOperationException(
					"only allow one initial state and addStartStates has already been called");
		}
	...
	public void setAPs(List<String> aps) throws HOAConsumerException {
		...
		for(int i = 0; i < aps.size(); i ++) {
			if(aps.get(i).equals(TAIL)) {
				tailVar = bdd.ithVar(i);
				break;
			}
		}
		System.out.println("tail: " + (tailVar == null));
...
//	      System.out.println(labelExpr);
		...
		// check whether it satisfies _tail
		assert tailVar != null : "tail is null";
		expr = expr.andWith(tailVar.id());
		...
		if(expr.isZero()) {
			indexStateMap.get(stateId).setAccept(true);
		}else {
